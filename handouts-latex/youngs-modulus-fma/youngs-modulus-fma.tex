...
[Y] = \frac{\mathrm{M}\mathrm{L}\mathrm{T}^{-2}}{\mathrm{L}^2} = \mathrm{M}\, \mathrm{L}^{-1}\, \mathrm{T}^{-2}.
\]
...
\pts{2}
\begin{pproblem}
Show, using base dimensions $\mathrm{M}$, $\mathrm{L}$, $\mathrm{T}$, that Young's modulus has the same dimensions as
energy per unit volume. (This is not a coincidence: stretching a wire stores elastic
energy, and $Y$ sets how much.)
\end{pproblem}
\begin{solution}
Energy has dimensions $\mathrm{M}\mathrm{L}^2\mathrm{T}^{-2}$ and volume has dimensions $\mathrm{L}^3$, so energy per volume
is $\mathrm{M}\mathrm{L}^2\mathrm{T}^{-2}/\mathrm{L}^3 = \mathrm{M}\mathrm{L}^{-1}\mathrm{T}^{-2}$. That is exactly the result computed above for $Y$.
\end{solution}
...
$Y$ has dimensions $\mathrm{M}\mathrm{L}^{-1}\mathrm{T}^{-2}$ and density $\rho$ has dimensions $\mathrm{M}\mathrm{L}^{-3}$. Divide:
\[
\left[\frac{Y}{\rho}\right] = \frac{\mathrm{M}\mathrm{L}^{-1}\mathrm{T}^{-2}}{\mathrm{M}\mathrm{L}^{-3}} = \mathrm{L}^2\mathrm{T}^{-2},
\]
...
\end{pproblem}
\begin{solution}
$YA$ has dimensions $(\mathrm{M}\mathrm{L}^{-1}\mathrm{T}^{-2})(\mathrm{L}^2) = \mathrm{M}\mathrm{L}\mathrm{T}^{-2}$, a force. Dividing by $\ell m$ gives
$\mathrm{M}\mathrm{L}\mathrm{T}^{-2}/(\mathrm{L}\mathrm{M}) = \mathrm{T}^{-2}$, so $\sqrt{YA/(\ell m)}$ is a frequency --- the only one you can
...
$[Y] = \mathrm{M}\mathrm{L}^{-1}\mathrm{T}^{-2}$ and $[\rho] = \mathrm{M}\mathrm{L}^{-3}$, the ratio $Y/\rho$ has dimensions
$\mathrm{L}^2\mathrm{T}^{-2}$, so $v = \sqrt{Y/\rho}$. Numerically
