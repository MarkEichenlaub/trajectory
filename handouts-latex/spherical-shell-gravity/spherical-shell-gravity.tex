...
Everything we had was essentially right. The ring slice, the law of cosines, the
substitution, and the two square roots we ended up subtracting were all correct. The one
thing we missed is that those square roots simplify, since what's under them turns out to
be a perfect square:
\[
R^2 + r^2 + 2Rr = (R+r)^2,
\qquad
R^2 + r^2 - 2Rr = (R-r)^2 .
\]
So the roots are just $R+r$ and $|R-r|$, and the answer comes out in one more line. (A
couple of factors of 2 wandered off in the coefficient out front too. Those are at the
end.)
...
Here's the same calculation start to finish, followed by the two lines where ours went
off.
