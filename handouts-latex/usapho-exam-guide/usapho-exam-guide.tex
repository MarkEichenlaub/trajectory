...
the F = ma exam in February. The USAPhO itself is in April and is proctored in person. The
questions show up on a computer screen, but you write your solutions by hand on blank paper
from the proctor, and those pages get scanned and graded by people. You get a reference
sheet with constants and a few approximations. You can use a basic scientific calculator.
Almost nobody finishes, so most of your score comes from partial credit.
\end{keybox}
...
of 25. You can't register for the USAPhO directly. See the \emph{F = ma Exam FAQ} handout
for more about that exam.
...
the break you can't go back to Part A, since those pages have already been collected. Think
of them as two separate 90-minute exams.
\end{warnbox}
...
isn't mechanics-only. It also expects calculus: you'll integrate, differentiate, expand in
...
So the problems come to you online, but you're still writing physics on paper for a person
to read.
...
used''), and it all gets collected at the end.
...
\begin{tipbox}
\textbf{Scratch paper is for deciding.} Scratch paper is a good place to try three
approaches and see which ones fall apart. Once you know which one works, you can switch to
...
That's the whole sheet. It's really a list of constants. It doesn't have
$\tfrac{1}{2}mv^2$, Maxwell's equations, moments of inertia, thermodynamic relations, or
the lens equation. Some years have also had a short table of integrals and trig identities,
but the 2026 sheet didn't. You need to know your formulas.

The four approximations are a hint about what the exam likes. Expansions in a small
parameter show up constantly. When a problem gives you a condition like $r \ll R$, it's
...
the exam. So nothing that does symbolic math, programming, or graphing. You can't share
calculators, and the proctor won't have one, so bring your own. You'll use it much less than
on F = ma, since most answers are symbolic.
...
estimating uncertainties. Data analysis is also in the USAPhO syllabus, so it's a good idea
to practice it.
...
They aren't trying to reconstruct your thinking, just to find those milestones.
...
the structure of what you did. A page of decimals doesn't show them anything. Also, one
...
That last line is the one that matters most: the problem number, then page $n$ of $N$ for
that problem. The scans get split up and sorted by problem before grading. A page labeled
only ``2'' can end up in the wrong pile, and a page in the wrong pile gets a zero.
...
None of these are hard rules. Graders are physicists reading in good faith, and if your $R$
is obviously the radius in your figure, nobody is going to pretend otherwise. But everything
in the right-hand column makes them work harder to find the points you earned, and across a
stack of three hundred papers that adds up.
...
wrong one. It only takes a second to cross something out.
\end{tipbox}

\subsection*{Getting past a part you can't do}

This habit helps on a lot of problems.
...
\end{center}
and then do (b) properly. You can still get full credit on (b). Two more versions of the
same move:
\begin{itemize}\setlength{\itemsep}{2pt}
...
\textbf{Check limiting cases on the page.} When you get an answer, check whether it reduces
to the earlier part when the correction goes to zero, whether the sign is right, and whether
the units work. Writing ``as $h \to 0$ this reduces to part (a) \checkmark'' does two
things: it catches your own mistakes, and it shows a grader who is deciding between two
rubric levels that you understand what you found.
...
reader can follow. A grader only spends a few minutes on your page, so structure helps.
...
That's five minutes out of ninety. It protects you from spending forty minutes on the
hardest problem of the set because it happened to be printed first, and never reading a
problem you could have solved.
...
assumed result. Some of the easiest points on the exam are in the last subpart of a hard
problem.

\subsection*{A time budget}

Ninety minutes for three problems is about 30 minutes each. The hard part is actually
...
\begin{tipbox}
\textbf{Stop at 30 minutes per problem.} When you hit 30 minutes on a problem, write down
where you got to and box whatever partial result you have, even if you're in the middle of
a derivation. If you have time at the end you can come back to it, and you'll have a fresh
head. The alternative, spending 50 minutes on one problem, costs you the easy first
subparts of a problem you never got to read.

\textbf{A five-minute rule for a stuck subpart:} if five minutes of real effort hasn't
produced a new equation, it's time to write down what you know, assume a result, and go to
the next subpart.
\end{tipbox}

\subsection*{The last four minutes of each part}

In the last few minutes, stop doing physics and clean up your pages:
...
It's easy to treat an unfinished problem as a failure and get rattled. But the USAPhO is
designed so that even the top students in the country don't finish it.

\begin{keybox}
Solving two problems completely and making real progress on two more is an excellent
performance. Leaving a few subparts blank is normal. Your goal is to collect as many points
as you can in 180 minutes, which is different from finishing the exam. With that goal, it
makes sense to leave a hard problem, read the next one, and pick up its first two subparts.
\end{keybox}
...
(median 46\% of the points) and 2017 was brutal (median 25\%). So there's no fixed number
to aim for. It's better to think in terms of what fraction of the exam you can do.
...
Half the field scored nothing at all on A3 and on B2. A3 was also the hardest problem in
Part A by a wide margin, and B3 was worth nearly twice what B2 was. A student who worked
strictly in order and got stuck on A3 missed the two problems in Part A that had the most
points.
\end{tipbox}
...
\ann{$\uparrow$ Every symbol used below ($P$, $\theta$, cm, $R$) is defined on the diagram.
The first line says which frame we're in.}
...
\ann{$\uparrow$ The principle (torque about $P$) is named, and the reason $N$ and $f$ drop
out takes half a line. That's a rubric line.}
...
\ann{$\uparrow$ This check is what caught the swapped lever arms. It took maybe thirty
seconds, and it's the difference between 8/8 and 3/8 on this part.}
...
\ann{$\uparrow$ The first check shows the correction is consistent with (a). The second
shows the sign of the correction was understood, not just stumbled into.}
...
That submission gets full credit, even though very little of the page is polished. There's
a crossed-out formula, an abandoned idea, and hardly any prose. It gets a 25 because every
rubric line is easy to find.

\subsection*{The same physics, written four other ways}

All four of these students did roughly the same amount of physics.
...
\begin{keybox}
The third student got \emph{part (a) wrong} and still scored 22, since subparts are graded
independently and (b) was written out. The fourth student got everything right but scored
6 because the work was on scratch paper. The difference between 6 and 22 here has nothing
to do with physics.
\end{keybox}
...
Try the limiting cases, which is what caught the swapped lever arms above. Set each
parameter to zero or infinity and see whether the answer still makes sense. If a check
fails, you've found a real error. If they all pass, there's no reason to keep worrying about
it, especially since a wrong (a) doesn't sink (b) anyway.

\Q{You can't see how to set up the integral in (b).}
Write down what you \emph{do} know (``the acceleration varies along the rod, so
$\tau_c = \int \omega^2 r z \, dm$''), then define $r$, $z$, and $dm$ one at a time from
your diagram. That setup alone gets 7 of the 17 points in (b), before you've integrated
anything.
...
and it's correct. If it passes the limiting cases, it's probably fine, and there's no reason
to spend ten minutes hunting for a mistake just because you expected something prettier.

\Q{You have two versions of an answer and no time to decide.}
Pick the one that passes your limiting checks and cross out the other one.

\Q{You're 30 minutes in with (b) half done.}
Box whatever you have and write one line saying what's left (``still need to evaluate the
integral and balance against gravity''). Then go read the next problem. Its first subpart
is probably worth more points than your next ten minutes on this one.

\section{Part VI: Two Real Papers}

The example above was made up. These two are real 2018 USAPhO submissions to Problem B2,
reproduced as the graders received them, with the scores they actually got. The scans only
carry the AAPT numbers printed on the answer sheet, so there are no names on them.
...
Notice the italicized sentence in part (b). The exam is telling you that you can use the
move from Part II. Only one of these two students took the offer.
...
carried through correctly \emph{from their own} (a). This is normal. Graders give credit
for a later part done correctly with a wrong number from an earlier part.
...
would probably have caught the mistake, since the missing $\langle r\rangle^2$ term is
visible as soon as the formula is on the page. Naming the principle first is also how you
...
The same student scored 25/25 on A1 and 23/25 on A3 that day, and A3 was the problem where
45\% of the field scored zero. So this student knew plenty of physics.
...
page connects them. Part (b) never uses the uncertainty principle, so there's no bound on
$\Delta\theta$. Part (c) finds the angle the telescope has to resolve and stops there,
without ever getting to the size of the telescope, which is what the question asked for.
Nothing is boxed.

\textbf{What would have earned more.} Three lines, none of which needed a new idea:
...
and it answers the question. An order-of-magnitude answer gets credit, but an intermediate
quantity doesn't get much.
\item \textbf{Put a sentence next to each sketch.} Three of the drawings on this page have
nothing written next to them. The (c) sketch has exactly the right idea in it (the star's
radius over its distance is the angle you need to resolve), but a grader can only give
credit for reasoning they can identify. A few words next to the sketch would have done it.
\end{itemize}

\begin{keybox}
Both students had the same two facts on the page. One turned them into two boxed numbers
and scored 22, and the other left them unconnected and scored 5. The difference wasn't how
much physics they knew.

Paper 2 isn't unusual. B2's mean score was 5.0 out of 25, and 52\% of the field scored zero
on it, even though the exam had told everyone, right in part (b), how to get past the step
that stopped them.
\end{keybox}
