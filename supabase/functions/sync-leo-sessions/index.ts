...
Deno.serve(async (req) => {
  ...
  const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_KEY, {
    global: { headers: { 'x-app-source': 'sync-leo-sessions' } },
  })
  ...
  const listing = await listCalendarEvents(
    accessToken, 'Leo / Mark Physics', now.toISOString(), maxDate.toISOString(),
  )
  if (!listing.ok) {
  ...
  for (const event of events) {
    ...
    if (!existing) {
      ...
      if (byEventId) {
        ...
        moved = true
        // Fall through rather than `continue`: a moved session still needs its
        // Meet link refreshed, and needs a board (plus sharing + calendar link)
        // if it never got one.
        existing = byEventId
      }
    }
    ...
    if (existing?.miro_board_id) {
      ...
      // Self-heal the two things that used to be done only on the pass that
      // created the board: the board was left `private` (student locked out)
      // and the invite never got the link. Both are idempotent.
    ...
    // Create Miro board (shared helper sets link-can-edit so Leo can open it)
    let miroBoardUrl = ''
    let miroBoardId = ''
    try {
    ...
    // Put the board link in the invite so Leo gets it with the event
